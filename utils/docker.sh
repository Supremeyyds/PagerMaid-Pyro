...
start_docker () {
    ...
    case $PGM_WEB in
        true)
            docker run -dit --restart=always --name="$container_name" --hostname="$container_name" -e WEB_ENABLE="$PGM_WEB" -e WEB_SECRET_KEY="$admin_password" -e WEB_HOST=0.0.0.0 -e WEB_PORT=50400 -e WEB_LOGIN="$PGM_WEB_LOGIN" -p 50400:50400 teampgm/pagermaid_pyro <&1
            ;;
        *)
        ...
        [yY][eE][sS] | [yY])
            ...
            if [ -d "$data_path" ]; then
                ...
                if docker inspect "$container_name" &>/dev/null; then
                    ...
                    case $PGM_WEB in
                        true)
                            docker run -dit -v "$data_path"/workdir:/pagermaid/workdir --restart=always --name="$container_name" --hostname="$container_name" -e WEB_ENABLE="$PGM_WEB" -e WEB_SECRET_KEY="$admin_password" -e WEB_HOST=0.0.0.0 -e WEB_PORT=50400 -p 50400:50400 teampgm/pagermaid_pyro <&1
                            ;;
                        *)
